...
**What do violin plots reveal that box plots do not?** Instead of the data being forced into a box format, it is spread to show the shape of how the actually varies across its spectrum



**What features are apparent in box plots but not in violin plots?** Single dot indicators for smaller values, as well as a highlight statistic within the box represented by a line.
...
Mismanaged plastic waste is much higher and more spread out than plastic wste per cap is.
...
**Are there clear distinctions between continents?** It's easy to see that North America, Africa, and Asia have the most recorded plastic waste.
...
The total pop and coastal pop have similar data spreads
...
plastic_waste %>% 
  mutate(coastal_pop_prop = coastal_pop / total_pop) %>%
  filter(plastic_waste_per_cap < 3) %>%
  ggplot(aes(x = coastal_pop_prop, y = coastal_pop, color = continent)) + 
    geom_point() +
    geom_smooth(color = "black") +
    scale_color_viridis_d() +
    labs(x = "coastal population proportion", 
         y = "plastic waste per capita", 
         color = "continent",
         title = "coastal population proportion of plastic waste",
         subtitle = "by continent") +
    theme_minimal()
```
...
The spread of plastic waste is pretty even across the population. Africa, Asia, and North America are still the outliers with higher per cap statistics.
